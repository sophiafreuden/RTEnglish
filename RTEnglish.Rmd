...
sums2 %>%
  ...
  group_by(ArtDate) %>% 
  summarise(Frequency = sum(putin)) %>%
  ggplot(aes(x = ArtDate, y = Frequency)) +
  geom_point() +
  geom_smooth(method = 'loess') +
  labs(
    title = "Term Frequency Per Article Over Time",
    subtitle = "Term: 'putin', RT Search Term: 'clinton maidan'",
    x = "Date",
    y = "Frequency"
  ...
  ggsave("clinton-maidan/putin.png", width = 10)
```
...
sum2 %>%
  ...
  tab_header(
    ...
    subtitle = "RT Search Term: 'soros open society'"
    )  %>% 
  # #tab_source_note(
