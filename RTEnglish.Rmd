...
data <- read_csv("stop-soros.csv", col_types = col_types)
...
sums2d %>%
  ...
  group_by(ArtDate) %>% 
  summarise(Frequency = sum(campaign)) %>%
  ggplot(aes(x = ArtDate, y = Frequency)) +
  geom_point() +
  geom_smooth(method = 'loess') +
  labs(
    title = "Term Frequency Per Article Over Time",
    subtitle = "Term: 'campaign', RT Search Term: 'stop soros'",
    x = "Date",
    y = "Frequency",
    # Comment/uncomment caption below as needed. Add/delete comma in line above, too.
    caption = "RT Search Term was entered with quotation marks for accuracy. Stemmed."
  ) +
...
```{r}
wordcloud(words = head3$term, freq = head3$count, min.freq = 1000,
  max.words=100, random.order=FALSE, rot.per=0.35, 
  colors=brewer.pal(8, "Dark2"))
```
...
sum2 %>%
  ...
  tab_header(
    ...
    subtitle = "RT Search Term: 'stop soros'"
    )  %>% 
  cols_label(
    V1 = "Min.",
    V2 = "1st Qu.",
