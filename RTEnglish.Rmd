...
sums2d %>%
  ...
  group_by(ArtDate) %>% 
  summarise(Frequency = sum(money)) %>%
  ggplot(aes(x = ArtDate, y = Frequency)) +
  geom_point() +
  geom_smooth(method = 'loess') +
  labs(
    title = "Term Frequency Per Article Over Time",
    subtitle = "Term: 'money,' RT Search Term: 'soros elections'",
    x = "Date",
    y = "Frequency",
  ...
  ggsave("soros-elections/money.png", width = 10)
```
...
```{r}
wordcloud(words = head$term, freq = head$count, min.freq = 150,
