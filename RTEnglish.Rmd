...
data <- read_csv("soros-democratic-party.csv", col_types = col_types)
...
sums2 %>%
  ...
  group_by(ArtDate) %>% 
  summarise(Frequency = sum(democraticparty)) %>%
  ggplot(aes(x = ArtDate, y = Frequency)) +
  geom_point() +
  geom_smooth(method = 'loess') +
  labs(
    title = "Term Frequency Per Article Over Time",
    subtitle = "Term: 'democraticparty', RT Search Term: 'soros democratic party'",
    x = "Date",
    y = "Frequency"
...
```{r}
wordcloud(words = head3$term, freq = head3$count, min.freq = 25,
  max.words=100, random.order=FALSE, rot.per=0.35, 
  colors=brewer.pal(8, "Dark2"))
```
...
sum2 %>%
  ...
  tab_header(
    ...
    subtitle = "RT Search Term: 'soros democratic party'"
    )  %>% 
  # #tab_source_note(
