...
data <- read_csv("mcfaul-civil-society.csv", col_types = col_types)
...
sums2d %>%
  ...
  group_by(ArtDate) %>% 
  summarise(Frequency = sum(ukrain)) %>%
  ggplot(aes(x = ArtDate, y = Frequency)) +
  geom_point() +
  geom_smooth(method = 'loess') +
  labs(
    title = "Term Frequency Per Article Over Time",
    subtitle = "Term: 'ukrain', RT Search Term: 'mcfaul civil society'",
    x = "Date",
    y = "Frequency",
...
```{r}
wordcloud(words = head3$term, freq = head3$count, min.freq = 150,
  max.words=100, random.order=FALSE, rot.per=0.35, 
  colors=brewer.pal(8, "Dark2"))
```
...
sum2 %>%
  ...
  tab_header(
    ...
    subtitle = "RT Search Term: 'mcfaul civil society'"
    )  %>% 
  # #tab_source_note(
