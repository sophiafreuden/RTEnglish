...
data <- read_csv("orange-revolution.csv", col_types = col_types)
...
sums2d %>%
  ...
  group_by(ArtDate) %>% 
  summarise(Frequency = sum(agent)) %>%
  ggplot(aes(x = ArtDate, y = Frequency)) +
  geom_point() +
  geom_smooth(method = 'loess') +
  labs(
    title = "Term Frequency Per Article Over Time",
    subtitle = "Term: 'agent', RT Search Term: 'orange revolution'",
    x = "Date",
    y = "Frequency",
...
sum2 %>%
  ...
  tab_header(
    ...
    subtitle = "RT Search Term: 'orange revolution'"
    )  %>% 
  # #tab_source_note(
