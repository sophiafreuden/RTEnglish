...
# non-stemmed
choice2 <- sums2 %>% select(c(ArtDate, trump, URL))
choice2 <- unique(choice2)
#view(choice2)

# stemmed
# choice2d <- sums2d %>% select(c(ArtDate, soros, URL))
...
choice2d %>%
  ...
  labs(
    ...
    subtitle = "Term: 'western', RT Search Term: N/A, 'Soros' Terms",
    x = "Date",
    y = "Frequency",
  ...
  ggsave("all-soros/western.png", width = 10)
```
...
```{r}
wordcloud(words = head$term, freq = head$count, min.freq = 750,
