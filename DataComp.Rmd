...
```{r}
all_data <- bind_rows(data123456789, data10)
```

```{r}
u <- unique(all_data$URL)
# u is a character vector length 909; raw all_data length 1382.
```
...
```{r}
all_clinton <- bind_rows(data1, data10)
```

```{r}
u_clinton <- unique(all_clinton$URL)
# u_clinton is a character vector length 151; raw all_clinton length 154.
```

```{r}
# u_soros: 429; u_clinton: 151; mcfaul: 200; orange revolution: 162. The sums of these != u as there is some overlap between them (but not within them).
```
