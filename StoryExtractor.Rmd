...
# temp <- sums2 %>% select(c(ArtDate, soros, URL))
# temp <- unique(temp)
temp2 <- sums2d %>% select(c(ArtDate, influenc, URL))
temp2 <- unique(temp2)
view(temp2)
...
temp3 <- temp %>% arrange(desc(soros))
temp3 <- head(temp3, 12) # select enough so that the top 10 unique terms are in it
temp3 <- temp3[-c(3, 9), ] # filter out the duplicate rows with the lesser quantity
